...
class loginView extends PureComponent{
	...
	check(e){
		let user = this.state.user,
			...
			pwd = this.state.pwd
		if(name === "pwd") {
			let first = crypto.createHash('md5').update(e.target.value).digest('hex')
			let sec = crypto.createHash('md5').update(first + this.state.user.acount).digest('hex')
			pwd = sec
		}else if(name === "acount"){
			let first = crypto.createHash('md5').update(this.state.user.pwd).digest('hex')
			let sec = crypto.createHash('md5').update(first + e.target.value).digest('hex')
			pwd = sec
		}
	...
	logining(){
		if(this.state.login){
			...
			let data = {
				user: this.state.user.acount,
				pwd: (this.state.type ? this.state.user.pwd : this.state.pwd),
				mode: (this.state.type ? 2 : 1 )
			}
			console.log(data)
